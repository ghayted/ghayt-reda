...
cursor.execute('''CREATE TABLE IF NOT EXISTS horaires (
                    ...
                    temps_arrivee TEXT,
                    temps_depart TEXT,
                    heures_travail REAL, -- Ajout de la nouvelle colonne pour stocker les heures de travail
    ...
    try:
        ...
        for row in rows:
            ...
            if match[0]:
                ...
                calculer_heures_travail()  # Calculer les heures de travail après chaque pointage d'arrivée
                return
...
def calculer_heures_travail():
    try:
        cursor.execute("SELECT id, temps_arrivee, temps_depart FROM horaires WHERE temps_arrivee IS NOT NULL AND temps_depart IS NOT NULL")
        rows = cursor.fetchall()

        for row in rows:
            employe_id, temps_arrivee, temps_depart = row
            temps_arrivee = datetime.datetime.strptime(temps_arrivee, "%Y-%m-%d %H:%M:%S")
            temps_depart = datetime.datetime.strptime(temps_depart, "%Y-%m-%d %H:%M:%S")
            difference = temps_depart - temps_arrivee
            heures_travail = difference.total_seconds()  # Convertir en heures
            # Mise à jour de la base de données avec les heures de travail calculées
            cursor.execute("UPDATE horaires SET heures_travail = ? WHERE id = ?", (heures_travail, employe_id))
            conn.commit()
    except sqlite3.OperationalError as e:
        messagebox.showerror("Erreur de base de données", f"Erreur opérationnelle : {str(e)}")
    except Exception as ex:
        ...
        for row in rows:
            ...
            if match[0]:
                ...
                if resultat:
                    ...
                    calculer_heures_travail()  # Calculer les heures de travail après chaque pointage de départ
                    return
                else:
